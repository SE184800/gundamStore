...
- **Response:** `{ success, products: LightweightProduct[], meta }`. Mỗi sản phẩm có `price, oldPrice, finalPrice, stock, availability{status,inStock,canAddToCart,isPreorder}, groups[], collections[], flashSale`.
- **Auth:** Public.
...
- **Response:** `{ success, products[], meta: { page?, limit, count, total?, totalPages? } }`. Sản phẩm bị ẩn số tồn kho thật (chỉ có `availability`). Mỗi sản phẩm kèm `flashSale`.
- **Auth:** Public.
- **Frontend:** ✅ `StorefrontProductApiService.js`.

#### Giá bán thực tế (`price`/`finalPrice`) — thứ tự ưu tiên, dùng chung cho `GET /api/products/`, `GET /api/products/:key`, `GET /api/products/home` VÀ `POST /api/orders/`
- **Một hàm duy nhất:** `resolveProductCommercialPrice`/`decorateProductWithCommercialPrice` (`services/commercialPriceResolver.js`) — tính `price`/`finalPrice` theo đúng 1 thứ tự ưu tiên ở **cả 4 nơi trên**, không còn 4 cách tính khác nhau (trước đây chỉ khu Flash Sale — `GET /api/flash-sales/active` — áp giá Flash Sale; 3 nơi còn lại kể cả lúc tạo đơn đều bỏ qua Flash Sale hoàn toàn, có thể tính SAI giá lúc chốt đơn):
  1. **Flash Sale đang `LIVE` cho sản phẩm này** (ưu tiên cao nhất, tra qua `resolveLiveFlashSaleItems` trong `services/flashSalePricing.js` — cùng logic `computeCampaignStatus` mà `GET /api/flash-sales/active` dùng, tại đúng thời điểm request theo giờ Asia/Ho_Chi_Minh của server). Giá tính từ `discountType`/`discountValue` của `FlashSaleItem` áp trên `product.price` gốc (`services/flashSalePricing.js`, `computeFlashSalePrice`) — y hệt công thức `GET /api/flash-sales/active` dùng, để không lệch nhau dù xem ở đâu.
  2. Nếu không có Flash Sale LIVE → Promotion thường đang active (`Promotion`/`PromotionProduct`), ưu tiên theo `priority` rồi `discountAmount`.
  3. Nếu không có cả hai → giá hiệu lực bình thường (`ProductPrice` đang active nếu có, ngược lại `product.price`).
  - **Chỉ áp dụng cho sản phẩm không có variant active** — `FlashSaleItem` không có `variantId`, sản phẩm có variant vẫn lấy giá theo variant như trước (Promotion cũng vốn không áp cho variant).
  - Khi có Flash Sale LIVE, response kèm field `flashSale: { id, campaignId, discountType, discountValue, dailyStockLimit, countdownTarget, soldToday }` (giống `FlashSaleItem` ở `GET /api/flash-sales/active`) để Frontend hiển thị badge/đếm ngược ngay trên card sản phẩm — `null` nếu không có Flash Sale LIVE.
...
- **Response:** `{ success, product: { ...full product, availability, flashSale, preorder: { isPreorder, isOpen, eta, depositType, depositValue, depositRate, canOrder, slotRemaining } } }`. `depositType`/`depositValue` là cấu hình cọc thật của sản phẩm (`PERCENT` 1-100 hoặc `FIXED_AMOUNT` VNĐ/đơn vị) — **không còn là tỷ lệ cố định 30% dùng chung cho mọi sản phẩm**. `depositRate` (0-1) chỉ có giá trị khi `depositType=PERCENT`, giữ lại cho code cũ đọc dạng phân số; là `null` khi `FIXED_AMOUNT`. `flashSale` — xem mục "Giá bán thực tế" ở `GET /api/products/`. 404 nếu không thấy.
...
> Bảng dữ liệu hoàn toàn tách biệt khỏi `Promotion`/`PromotionProduct` (`FlashSaleCampaign`/`FlashSaleItem`, `flashSaleController.js`) — không dùng chung bảng. Nhưng **giá bán cuối cùng hiển thị cho khách** (`GET /api/products/`, `GET /api/products/:key`, `GET /api/products/home`, `POST /api/orders/`) giờ dùng chung 1 hàm ưu tiên Flash Sale trước Promotion (xem mục "Giá bán thực tế" ở `GET /api/products/`) — không còn là 2 hệ thống hiển thị tách rời như trước. Trạng thái luôn tính theo giờ **Asia/Ho_Chi_Minh cố định UTC+7** bất kể timezone server (`src/utils/vnTime.js`), vì Việt Nam không có DST.
...
- **Response:** `{ success, campaigns: [{ id, nameVi, nameEn, dateFrom, dateTo, windows: [{dailyStartTime, dailyEndTime}], status:"UPCOMING"|"LIVE", countdownTarget: ISOString, items: [{ id, productId, discountType, discountValue, finalPrice, dailyStockLimit, soldToday, product:{...} }] }] }`.
  ...
  - `countdownTarget`: quét qua tất cả `windows` theo thứ tự giờ bắt đầu — nếu đang trong 1 khung → `LIVE`, đếm ngược tới giờ kết thúc khung đó; nếu chưa tới khung gần nhất → `UPCOMING`, đếm ngược tới giờ bắt đầu khung đó; nếu đã qua hết khung hôm nay:
    - Nếu **ngày mai vẫn còn trong `[dateFrom, dateTo]`** của campaign → vẫn trả `UPCOMING`, `countdownTarget` = giờ bắt đầu của khung **sớm nhất** trong `windows`, tính cho **ngày mai** (không phải hôm nay) — campaign nhiều ngày không được biến mất khỏi response chỉ vì đã qua hết khung giờ của hôm nay, nó phải tiếp tục hiện ở trạng thái đếm ngược sang ngày kế tiếp.
